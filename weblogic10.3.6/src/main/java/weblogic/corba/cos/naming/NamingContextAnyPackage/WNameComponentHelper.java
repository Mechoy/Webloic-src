...
public abstract class WNameComponentHelper {
      ...
      if (__typeCode == null) {
         ...
         synchronized(TypeCode.class) {
            if (__typeCode == null) {
               ...
               TypeCode var2 = null;
               var2 = ORB.init().create_wstring_tc(0);
               var1[0] = new StructMember("id", var2, (IDLType)null);
               var2 = ORB.init().create_string_tc(0);
               var2 = ORB.init().create_alias_tc(IstringHelper.id(), "Istring", var2);
               var1[1] = new StructMember("kind", var2, (IDLType)null);
               __typeCode = ORB.init().create_struct_tc(id(), "WNameComponent", var1);
               __active = false;
            }
         }
      }
   ...
   public static WNameComponent read(InputStream var0) {
      WNameComponent var1 = new WNameComponent();
      var1.id = var0.read_wstring();
      var1.kind = var0.read_string();
      return var1;
   }

   public static void write(OutputStream var0, WNameComponent var1) {
      var0.write_wstring(var1.id);
      var0.write_string(var1.kind);
   }
}
