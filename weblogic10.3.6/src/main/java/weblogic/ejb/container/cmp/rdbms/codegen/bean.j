...
@start rule: writeOracleBlobToOutputStream
	if (lob instanceof weblogic.jdbc.vendor.oracle.OracleThinBlob) {
          // Oracle thin driver
	  os = ((weblogic.jdbc.vendor.oracle.OracleThinBlob) lob).getBinaryOutputStream();
	}
	else {
	  // DataDirect driver for Oracle
...
@start rule: getOracleClobWriter
	if (lob instanceof weblogic.jdbc.vendor.oracle.OracleThinClob) {
	  // Oracle thin driver
	  writer = ((weblogic.jdbc.vendor.oracle.OracleThinClob) lob).getCharacterOutputStream();
	}
	else {
	  // DataDirect driver for Oracle
