...
public class AdminConsoleMBeanBinder extends ConfigurationMBeanBinder implements AttributeBinder {
   private AdminConsoleMBeanImpl bean;

   protected AdminConsoleMBeanBinder(DescriptorBean var1) {
      super(var1);
      this.bean = (AdminConsoleMBeanImpl)var1;
   }

   public AdminConsoleMBeanBinder() {
      super(new AdminConsoleMBeanImpl());
      this.bean = (AdminConsoleMBeanImpl)this.getBean();
   }
         ...
         if (!(this instanceof SecurityReadOnlyMBeanBinder) && var2 != null && var2.toString().trim().length() == 0) {
            ...
            if (var1 != null) {
               if (var1.equals("CookieName")) {
                  try {
                     this.bean.setCookieName((String)var2);
                  } catch (BeanAlreadyExistsException var6) {
                     System.out.println("Warning: multiple definitions with same name: " + var6.getMessage());
                  }
               } else if (var1.equals("SessionTimeout")) {
                  try {
                     this.bean.setSessionTimeout(Integer.valueOf((String)var2));
