...
public class CacheLoaderMBeanBinder extends ConfigurationMBeanBinder implements AttributeBinder {
   private CacheLoaderMBeanImpl bean;

   protected CacheLoaderMBeanBinder(DescriptorBean var1) {
      super(var1);
      this.bean = (CacheLoaderMBeanImpl)var1;
   }

   public CacheLoaderMBeanBinder() {
      super(new CacheLoaderMBeanImpl());
      this.bean = (CacheLoaderMBeanImpl)this.getBean();
   }
         ...
         if (!(this instanceof SecurityReadOnlyMBeanBinder) && var2 != null && var2.toString().trim().length() == 0) {
            ...
            if (var1 != null) {
               if (var1.equals("CustomLoader")) {
                  try {
                     this.bean.setCustomLoader((String)var2);
