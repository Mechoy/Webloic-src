...
public class ErrorHandlingMBeanBinder extends ConfigurationMBeanBinder implements AttributeBinder {
   private ErrorHandlingMBeanImpl bean;

   protected ErrorHandlingMBeanBinder(DescriptorBean var1) {
      super(var1);
      this.bean = (ErrorHandlingMBeanImpl)var1;
   }

   public ErrorHandlingMBeanBinder() {
      super(new ErrorHandlingMBeanImpl());
      this.bean = (ErrorHandlingMBeanImpl)this.getBean();
   }
         ...
         if (!(this instanceof SecurityReadOnlyMBeanBinder) && var2 != null && var2.toString().trim().length() == 0) {
            ...
            if (var1 != null) {
               if (var1.equals("LogPolicy")) {
                  try {
                     this.bean.setLogPolicy((String)var2);
                  } catch (BeanAlreadyExistsException var6) {
                     System.out.println("Warning: multiple definitions with same name: " + var6.getMessage());
                  }
               } else if (var1.equals("Policy")) {
                  try {
                     this.bean.setPolicy((String)var2);
