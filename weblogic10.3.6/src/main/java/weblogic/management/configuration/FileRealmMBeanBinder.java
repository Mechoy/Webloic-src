...
public class FileRealmMBeanBinder extends ConfigurationMBeanBinder implements AttributeBinder {
   private FileRealmMBeanImpl bean;

   protected FileRealmMBeanBinder(DescriptorBean var1) {
      super(var1);
      this.bean = (FileRealmMBeanImpl)var1;
   }

   public FileRealmMBeanBinder() {
      super(new FileRealmMBeanImpl());
      this.bean = (FileRealmMBeanImpl)this.getBean();
   }
         ...
         if (!(this instanceof SecurityReadOnlyMBeanBinder) && var2 != null && var2.toString().trim().length() == 0) {
            ...
            if (var1 != null) {
               if (var1.equals("MaxACLs")) {
                  try {
                     this.bean.setMaxACLs(Integer.valueOf((String)var2));
                  } catch (BeanAlreadyExistsException var7) {
                     System.out.println("Warning: multiple definitions with same name: " + var7.getMessage());
                  }
               } else if (var1.equals("MaxGroups")) {
                  try {
                     this.bean.setMaxGroups(Integer.valueOf((String)var2));
                  } catch (BeanAlreadyExistsException var6) {
                     System.out.println("Warning: multiple definitions with same name: " + var6.getMessage());
                  }
               } else if (var1.equals("MaxUsers")) {
                  try {
                     this.bean.setMaxUsers(Integer.valueOf((String)var2));
