...
public class JMSJDBCStoreMBeanBinder extends JMSStoreMBeanBinder implements AttributeBinder {
   private JMSJDBCStoreMBeanImpl bean;

   protected JMSJDBCStoreMBeanBinder(DescriptorBean var1) {
      super(var1);
      this.bean = (JMSJDBCStoreMBeanImpl)var1;
   }

   public JMSJDBCStoreMBeanBinder() {
      super(new JMSJDBCStoreMBeanImpl());
      this.bean = (JMSJDBCStoreMBeanImpl)this.getBean();
   }
         ...
         if (!(this instanceof SecurityReadOnlyMBeanBinder) && var2 != null && var2.toString().trim().length() == 0) {
            ...
            if (var1 != null) {
               if (var1.equals("ConnectionPool")) {
                  ...
                  try {
                     this.bean.setCreateTableDDLFile((String)var2);
                  } catch (BeanAlreadyExistsException var7) {
                     System.out.println("Warning: multiple definitions with same name: " + var7.getMessage());
                  }
               } else if (var1.equals("DelegatedBean")) {
                  this.bean.setDelegatedBeanAsString((String)var2);
