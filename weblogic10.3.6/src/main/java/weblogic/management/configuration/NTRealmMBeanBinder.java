...
public class NTRealmMBeanBinder extends BasicRealmMBeanBinder implements AttributeBinder {
   private NTRealmMBeanImpl bean;

   protected NTRealmMBeanBinder(DescriptorBean var1) {
      super(var1);
      this.bean = (NTRealmMBeanImpl)var1;
   }

   public NTRealmMBeanBinder() {
      super(new NTRealmMBeanImpl());
      this.bean = (NTRealmMBeanImpl)this.getBean();
   }
         ...
         if (!(this instanceof SecurityReadOnlyMBeanBinder) && var2 != null && var2.toString().trim().length() == 0) {
            ...
            if (var1 != null) {
               if (var1.equals("IgnoreBadDomainName")) {
                  try {
                     this.bean.setIgnoreBadDomainName(Boolean.valueOf((String)var2));
                  ...
                  try {
                     this.bean.setPreferredMaxBytes(Integer.valueOf((String)var2));
                  } catch (BeanAlreadyExistsException var6) {
                     System.out.println("Warning: multiple definitions with same name: " + var6.getMessage());
                  }
               } else if (var1.equals("PrimaryDomain")) {
                  try {
                     this.bean.setPrimaryDomain((String)var2);
