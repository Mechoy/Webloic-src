...
public class SNMPLogFilterMBeanBinder extends SNMPTrapSourceMBeanBinder implements AttributeBinder {
   private SNMPLogFilterMBeanImpl bean;

   protected SNMPLogFilterMBeanBinder(DescriptorBean var1) {
      super(var1);
      this.bean = (SNMPLogFilterMBeanImpl)var1;
   }

   public SNMPLogFilterMBeanBinder() {
      super(new SNMPLogFilterMBeanImpl());
      this.bean = (SNMPLogFilterMBeanImpl)this.getBean();
   }
         ...
         if (!(this instanceof SecurityReadOnlyMBeanBinder) && var2 != null && var2.toString().trim().length() == 0) {
            ...
            if (var1 != null) {
               if (var1.equals("MessageIds")) {
                  try {
                     this.bean.setMessageIds(this.parseStringArrayInitializer((String)var2));
                  } catch (BeanAlreadyExistsException var9) {
                     System.out.println("Warning: multiple definitions with same name: " + var9.getMessage());
                  }
               } else if (var1.equals("MessageSubstring")) {
                  try {
                     this.bean.setMessageSubstring((String)var2);
                  } catch (BeanAlreadyExistsException var8) {
                     System.out.println("Warning: multiple definitions with same name: " + var8.getMessage());
                  }
               } else if (var1.equals("SeverityLevel")) {
                  try {
                     this.bean.setSeverityLevel((String)var2);
