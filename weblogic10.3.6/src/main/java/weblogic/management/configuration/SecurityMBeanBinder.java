...
public class SecurityMBeanBinder extends ConfigurationMBeanBinder implements AttributeBinder {
   private SecurityMBeanImpl bean;

   protected SecurityMBeanBinder(DescriptorBean var1) {
      super(var1);
      this.bean = (SecurityMBeanImpl)var1;
   }

   public SecurityMBeanBinder() {
      super(new SecurityMBeanImpl());
      this.bean = (SecurityMBeanImpl)this.getBean();
   }
         ...
         if (!(this instanceof SecurityReadOnlyMBeanBinder) && var2 != null && var2.toString().trim().length() == 0) {
            ...
            if (var1 != null) {
               if (var1.equals("AuditProviderClassName")) {
                  try {
                     this.bean.setAuditProviderClassName((String)var2);
                  } catch (BeanAlreadyExistsException var10) {
                     System.out.println("Warning: multiple definitions with same name: " + var10.getMessage());
                  }
               } else if (var1.equals("LogAllChecksEnabled")) {
                  try {
                     this.bean.setLogAllChecksEnabled(Boolean.valueOf((String)var2));
                  ...
                  this.bean.setPasswordPolicyAsString((String)var2);
               } else if (var1.equals("Realm")) {
                  this.bean.setRealmAsString((String)var2);
               } else if (var1.equals("SystemUser")) {
                  try {
                     this.bean.setSystemUser((String)var2);
                  } catch (BeanAlreadyExistsException var7) {
                     System.out.println("Warning: multiple definitions with same name: " + var7.getMessage());
                  }
               } else if (var1.equals("CompatibilityMode")) {
                  try {
                     this.bean.setCompatibilityMode(Boolean.valueOf((String)var2));
                  } catch (BeanAlreadyExistsException var6) {
                     System.out.println("Warning: multiple definitions with same name: " + var6.getMessage());
                  }
               } else if (var1.equals("GuestDisabled")) {
                  try {
                     this.bean.setGuestDisabled(Boolean.valueOf((String)var2));
