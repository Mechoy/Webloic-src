...
public class WTCExportMBeanBinder extends ConfigurationMBeanBinder implements AttributeBinder {
   private WTCExportMBeanImpl bean;

   protected WTCExportMBeanBinder(DescriptorBean var1) {
      super(var1);
      this.bean = (WTCExportMBeanImpl)var1;
   }

   public WTCExportMBeanBinder() {
      super(new WTCExportMBeanImpl());
      this.bean = (WTCExportMBeanImpl)this.getBean();
   }
         ...
         if (!(this instanceof SecurityReadOnlyMBeanBinder) && var2 != null && var2.toString().trim().length() == 0) {
            ...
            if (var1 != null) {
               if (var1.equals("EJBName")) {
                  try {
                     this.bean.setEJBName((String)var2);
                  ...
                  try {
                     this.bean.setTargetClass((String)var2);
                  } catch (BeanAlreadyExistsException var6) {
                     System.out.println("Warning: multiple definitions with same name: " + var6.getMessage());
                  }
               } else if (var1.equals("TargetJar")) {
                  try {
                     this.bean.setTargetJar((String)var2);
