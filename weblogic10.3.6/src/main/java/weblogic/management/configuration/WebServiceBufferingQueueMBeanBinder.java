...
public class WebServiceBufferingQueueMBeanBinder extends ConfigurationMBeanBinder implements AttributeBinder {
   private WebServiceBufferingQueueMBeanImpl bean;

   protected WebServiceBufferingQueueMBeanBinder(DescriptorBean var1) {
      super(var1);
      this.bean = (WebServiceBufferingQueueMBeanImpl)var1;
   }

   public WebServiceBufferingQueueMBeanBinder() {
      super(new WebServiceBufferingQueueMBeanImpl());
      this.bean = (WebServiceBufferingQueueMBeanImpl)this.getBean();
   }
         ...
         if (!(this instanceof SecurityReadOnlyMBeanBinder) && var2 != null && var2.toString().trim().length() == 0) {
            ...
            if (var1 != null) {
               if (var1.equals("ConnectionFactoryJndiName")) {
                  ...
                  try {
                     this.bean.setEnabled(new Boolean((String)var2));
                  } catch (BeanAlreadyExistsException var6) {
                     System.out.println("Warning: multiple definitions with same name: " + var6.getMessage());
                  }
               } else if (var1.equals("TransactionEnabled")) {
                  try {
                     this.bean.setTransactionEnabled(new Boolean((String)var2));
