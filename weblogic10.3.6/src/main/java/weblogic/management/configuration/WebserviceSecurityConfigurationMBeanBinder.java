...
public class WebserviceSecurityConfigurationMBeanBinder extends ConfigurationMBeanBinder implements AttributeBinder {
   ...
   protected WebserviceSecurityConfigurationMBeanBinder(DescriptorBean var1) {
      super(var1);
      this.bean = (WebserviceSecurityConfigurationMBeanImpl)var1;
   }

   public WebserviceSecurityConfigurationMBeanBinder() {
      super(new WebserviceSecurityConfigurationMBeanImpl());
      this.bean = (WebserviceSecurityConfigurationMBeanImpl)this.getBean();
   }
      ...
      try {
         ...
         if (!(this instanceof SecurityReadOnlyMBeanBinder) && var2 != null && var2.toString().trim().length() == 0) {
            ...
            if (var1 != null) {
               ...
               } else if (var1.equals("ConfigurationProperty")) {
                  try {
                     this.bean.addConfigurationProperty((ConfigurationPropertyMBean)((ReadOnlyMBeanBinder)var2).getBean());
                  } catch (BeanAlreadyExistsException var6) {
                     System.out.println("Warning: multiple definitions with same name: " + var6.getMessage());
                     this.bean.removeConfigurationProperty((ConfigurationPropertyMBean)var6.getExistingBean());
                     this.bean.addConfigurationProperty((ConfigurationPropertyMBean)((AbstractDescriptorBean)((ConfigurationPropertyMBean)((ReadOnlyMBeanBinder)var2).getBean()))._cloneIncludingObsolete());
                  }
               } else if (var1.equals("TokenType")) {
                  try {
                     this.bean.setTokenType((String)var2);
