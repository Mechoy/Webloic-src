...
public class BrowseHelper {
   ...
   static String nextCdValue = "";
   static boolean incrementCdCount = false;
   ...
   void populateNames(Object obj) throws ScriptException {
      try {
         if (obj instanceof ObjectName) {
            try {
               if (!this.ctx.inNewTree()) {
                  this.ctx.wlInstanceObjName_name = (String)this.ctx.mbs.getAttribute((ObjectName)obj, "Name");
               } else {
            ...
            for(int i = 0; i < this.ctx.wlInstanceObjNames.length; ++i) {
               try {
                  if (!this.ctx.inNewTree()) {
                     this.ctx.wlInstanceObjNames_names[i] = (String)this.ctx.mbs.getAttribute(this.ctx.wlInstanceObjNames[i], "Name");
                  } else {
         ...
         this.ctx.throwWLSTException("Error populating the object names", var6);
      }
   ...
   boolean checkNameStartsWithSlash(String mname) throws ScriptException {
      try {
         ...
         this.ctx.throwWLSTException("Error while checking slashes in the MBean names", var6);
      }
