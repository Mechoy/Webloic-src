...
public class EditHandler {
   ...
   public EditHandler(WLScriptContext ctx) {
      ...
      this.baseSecurityProviderAttributeTypes.add("KeyStore");
   ...
   boolean set(String attrName, Object value) throws ScriptException {
      ...
      try {
         ...
         MBeanServerConnection connection = this.ctx.getMBSConnection((String)null);
         Attribute attr = new Attribute(attrName, this.getRealObject(attrName, value));
   ...
   Object invoke(String methodName, Object[] parameters, String[] signatures) throws ScriptException {
      try {
         ...
         this.ctx.throwWLSTException(this.txtFmt.getErrorOnInvoke(), var6);
      }
            ...
            if (creator != null) {
               try {
                  ...
                  if (!(var13 instanceof InvocationTargetException)) {
                     throw (ScriptException)var13;
                  }

                  InvocationTargetException ite = (InvocationTargetException)var13;
                  ...
                  if (((MBeanException)udTh.getUndeclaredThrowable()).getTargetException() instanceof BeanAlreadyExistsException) {
                     ...
                     this.ctx.throwWLSTException(msg, var13.getCause());
                  } else {
                     WLScriptContext var10001 = this.ctx;
                     this.ctx.throwWLSTException("Unexpected Error: ", var13);
                  }
               }
               ...
               try {
                  ...
                  this.ctx.throwWLSTException(this.txtFmt.getCannotCreateUseEditTree(), var11);
               }
            }
   ...
   private Method findDestroyMethod(String destroyName, Class clz) {
      ...
      Class[] arr$ = superInterfaces;
      int len$ = superInterfaces.length;
      int i$ = 0;

      while(i$ < len$) {
         Class superInterface = arr$[i$];
   ...
   private Object nowCreateSecurityMBean(String name, String providerType, String baseProviderType) throws ScriptException {
      try {
         ...
         if (baseProviderType.equals("KeyStore")) {
   ...
   private void deleteSecurityMBean(String name, String baseProviderType) throws ScriptException {
      ...
      } else if (baseProviderType.equals("KeyStore")) {
   ...
   WebLogicMBean getTarget(String path) throws ScriptException {
      if (!this.checkForAdminServer("getTarget")) {
         return null;
      } else if (!this.checkForCompatibilityTree()) {
         ...
         try {
            ...
            String[] paths = StringUtils.splitCompletely(path, "/");
            if (paths.length != 2) {
            ...
            do {
               ...
               mbean = (WebLogicMBean)iter.next();
            } while(!mbean.getName().equals(name));

            return mbean;
         } catch (InstanceNotFoundException var8) {
            if (this.ctx.debug) {
               this.ctx.throwWLSTException(this.txtFmt.getInstanceNotFound(path), var8);
            }
         } catch (Throwable var9) {
            if (var9 instanceof ScriptException) {
               throw (ScriptException)var9;
            ...
            this.ctx.throwWLSTException(this.txtFmt.getErrorGettingMBean(), var9);
         }
   ...
   private boolean isCreatePossible(String mbeanType, String childType) throws ScriptException {
      ...
      try {
         ...
         this.ctx.throwWLSTException("Error determining if creating an MBean is possible", var8);
      }
   ...
   private String getCreatorOrDestroyer(String attributeName, String whatever) throws ScriptException {
      try {
         ...
         this.ctx.throwWLSTException("Error getting the creator/destroyer", var10);
      }
   ...
   private String getInterfaceClassName(String mbeanType) throws ScriptException {
      try {
         ...
         this.ctx.throwWLSTException("Error getting the interfaceclassname", var9);
      }
