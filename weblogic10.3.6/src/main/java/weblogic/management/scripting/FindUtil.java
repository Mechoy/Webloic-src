...
public class FindUtil {
   ...
   void scanForAttributes(String interfaceName, String attrName, String parentAttr) throws ScriptException {
      if (!this.didIScanYou.contains(interfaceName)) {
         ...
         try {
            ...
            this.ctx.throwWLSTException("Error in scanning for attributes", var11);
         }
   ...
   private void walkTheTree(ObjectName root, String name) throws ScriptException {
      ...
      try {
         ...
         this.ctx.throwWLSTException("Error getting the attributes for MBean", var14);
      }
         ...
         try {
            for(int i = 0; i < info.length; ++i) {
               ...
               if (attrInfo.getName().toLowerCase(Locale.US).indexOf(name.toLowerCase(Locale.US)) != -1) {
                  Pair pair = null;
                  if (this.ctx.infoHandler.isEncrypted(model_ainfo)) {
                  ...
                  if (retObj != null) {
                     ...
                     if (bean != null) {
                        try {
                           ...
                           System.out.println("%%%% an exception occured while walking " + bean + " ... " + var12);
            ...
            this.ctx.throwWLSTException("Error walking the tree", var15);
         }
   ...
   private List findMBeanInThisTree(String name, String type, MBeanServerConnection connection) throws ScriptException {
      ...
      try {
         ...
         this.ctx.throwWLSTException("Error finding the MBean", var12);
         return null;
      }
   }
