...
public abstract class ResourceBase implements SelfDescribingResourceV2 {
   ...
   protected static StringBuffer appendValue(StringBuffer buf, String value) {
      if (value == null) {
         ...
         int index = indexOfSpecialChar(value, start, end);
         if (index == end) {
            return buf.append(value);
         } else {
            while(true) {
               buf.append(value.substring(start, index));
               if (index >= end) {
                  return buf;
               }

               buf.append('\\').append(value.charAt(index));
               start = index + 1;
               if (start >= end) {
                  return buf;
               }

               index = indexOfSpecialChar(value, start, end);
            }
         }
      }
   }
   ...
   private static int indexOfSpecialChar(String s, int start, int end) {
      int result = end;

      for(int i = SPECIAL_CHARS.length - 1; i >= 0; --i) {
         int found = s.indexOf(SPECIAL_CHARS[i], start);
