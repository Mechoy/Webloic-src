...
public final class SSLSetup extends SSLSetupLogging {
   ...
   public static synchronized int getLicenseLevel() {
      if (licenseLevel > -1) {
            ...
            if (!isFatClient()) {
               ...
               if (null == var2) {
                  ...
                  if (null == var3) {
                     ...
                     SSLMBean var4 = var3.getSSL();
                     if (null == var4) {
                        debug(2, "Failure determining whether to enable JSSE, server SSLMBean is null for kernelId=" + kernelId + ".");
                     } else {
                        jsseEnabled = var4.isJSSEEnabled();
                     }
                  }
               }
            } else {
               jsseEnabled = Boolean.getBoolean("weblogic.security.SSL.enableJSSE");
            }
         } catch (RuntimeException var6) {
            debug(2, var6, "Failure determining whether to enable JSSE.");
            throw var6;
         }
         ...
         } catch (JSAFE_InvalidUseException var5) {
            debug(2, var5, "Failure switching from CryptoJ's FIPS140_MODE to FIPS140_SSL_MODE");
         }

         String var1;
         if (jsseEnabled) {
            var1 = "weblogic.security.SSL.jsseadapter.JaSSLContextImpl";
            info("Use JSSE SSL (default strength)");
         } else {
            var1 = "com.bea.sslplus.CerticomSSLContext";
   ...
   private static void setSSLDelegate(String var0) {
      try {
         ...
         if (!SSLContextDelegate.class.isAssignableFrom(sslDelegateClass)) {
            String var4 = "Cannot initialize SSL implementation. " + var0 + " does not implement " + SSLContextDelegate.class.getName();
      ...
      boolean var1 = Boolean.getBoolean("weblogic.security.SSL.sendEmptyCAList");
      info("Empty CA List is enabled :" + var1);
      if (var1) {
         TLSSystem.setSendEmptyCertRequest(var1);
      }
   ...
   public static int getProtocolVersion() {
      if (!protocolVersionChecked) {
         try {
            String var0 = CommandLine.getCommandLine().getSSLMinimumProtocolVersion();
            if (null != var0) {
               SSLEnabledProtocolVersionsLogging var1 = new SSLEnabledProtocolVersionsLogging();
               protocolVersion = SSLEnabledProtocolVersions.getSSLContextDelegateProtocolVersions(var0, var1);
            } else {
               String var3 = CommandLine.getCommandLine().getSSLVersion();
               if (var3 != null) {
                  if (var3.equalsIgnoreCase("SSL3")) {
                     protocolVersion = 1;
                  } else if (var3.equalsIgnoreCase("TLS1")) {
                     protocolVersion = 0;
               ...
               if (!var0.equalsIgnoreCase("off") && !var0.equalsIgnoreCase("false")) {
                  if (!var0.equalsIgnoreCase("strong") && !var0.equalsIgnoreCase("true")) {
                     if (var0.equalsIgnoreCase("strict")) {
                        ...
                        if (var0.equalsIgnoreCase("strong_nov1cas")) {
                           enforceConstraints = 4;
                        } else if (var0.equalsIgnoreCase("strict_nov1cas")) {
   ...
   public static void logCertificateChainNoV1CAFailure(SSLSocket var0) {
      String var1 = getPeerName(var0);
      debug(2, "The certificate chain received from " + var1 + " contained a V1 CA certificate which cannot be a CA");
      if (logSSLRejections()) {
         Loggable var2 = SecurityLogger.logCertificateChainNoV1CAFailureLoggable(var1);
         var2.log();
         setFailureDetails(var0.getSession(), var2.getMessage());
      }
